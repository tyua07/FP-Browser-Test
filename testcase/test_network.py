...
class TestNetwork(object):
    ...
    # @pytest.mark.skip()
    @pytest.mark.parametrize('value', [
        WebConnectionType.WIFI,
        WebConnectionType.CELLULAR,
    ])
    def test_type(self, value):
       ...
       '''
        value = self.driver.execute_script(script)
        setting_value = self.config.get('connection.type')

        print("设置 网络类型:", value, setting_value)
        assert value == setting_value
    ...
    def test_downlink_max(self, value):
        """
        测试 网络最大下行速度
        """
        settings = FPBrowserSettings()
       ...
       '''
        value = self.driver.execute_script(script)
        setting_value = self.config.get('connection.downlink-max')

        print("设置 网络最大下行速度:", value, setting_value)
